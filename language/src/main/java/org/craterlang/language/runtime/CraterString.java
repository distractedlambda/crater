...
public final class CraterString implements TruffleObject {
    ...
    private static int tryToParseFourDigits(byte[] a, int offset) {
        int chunk = ByteArraySupport.littleEndian().getInt(a, offset);
    ...
    private static final int MIN_EXP_POWER_OF_TEN = -325;
    private static final int MAX_EXP_POWER_OF_TEN = 308;

    @CompilerDirectives.CompilationFinal(dimensions = 1) private static final long[] MANTISSA_64 = {
    ...
    @CompilerDirectives.CompilationFinal(dimensions = 1) private static final long[] MANTISSA_128 = {
    ...
    @CompilerDirectives.CompilationFinal(dimensions = 1) private static final double[] DOUBLE_POWERS_OF_TEN = {
        1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e11,
        1e12, 1e13, 1e14, 1e15, 1e16, 1e17, 1e18, 1e19, 1e20, 1e21, 1e22
    };

    private static double tryDecFloatToDoubleWithFastAlgorithm(boolean isNegative, long significand, int power) {
        ...
        long factorMantissa = MANTISSA_64[power - MIN_EXP_POWER_OF_TEN];
        ...
        long lower = shiftedSignificand * factorMantissa;
        long upper = multiplyHighUnsigned(shiftedSignificand, factorMantissa);
        if ((upper & 0x1ffL) == 0x1ffL && Long.compareUnsigned(lower + shiftedSignificand, lower) < 0) {
            long factorMantissaLow = MANTISSA_128[power - MIN_EXP_POWER_OF_TEN];
            long productLow = shiftedSignificand * factorMantissaLow;
            long productMiddle2 = multiplyHighUnsigned(shiftedSignificand, factorMantissaLow);
            ...
            if (((productMiddle + 1 == 0) && ((productHigh & 0x1ffL) == 0x1ffL) && (productLow + Long.compareUnsigned(shiftedSignificand, productLow) < 0))) {
                return Double.NaN;
            }
            upper = productHigh;
        }
        ...
        if ((realExponent < 1) || (realExponent > Double.MAX_EXPONENT + DOUBLE_EXPONENT_BIAS)) {
            return Double.NaN;
        }

        long bits = mantissa | realExponent << (DOUBLE_SIGNIFICAND_WIDTH - 1) | (isNegative ? 1L << 63 : 0L);
        return Double.longBitsToDouble(bits);
    }

    private static double tryHexFloatToDoubleWithFastAlgorithm(boolean isNegative, long significand, int power) {
    ...
    private static final int MAX_INPUT_LENGTH = Integer.MAX_VALUE - 4;
    private static final long MINIMAL_NINETEEN_DIGIT_INTEGER = 1000_00000_00000_00000L;
    private static final int MAX_EXPONENT_NUMBER = 1024;
    private static final byte DECIMAL_POINT_CLASS = -4;
    private static final byte OTHER_CLASS = -1;

    @CompilerDirectives.CompilationFinal(dimensions = 1) private static final byte[] CHAR_TO_HEX_MAP = new byte[128];
    ...
    private static double slowPath(byte[] bytes, int offset, int length) {
        return Double.parseDouble(new String(bytes, offset, length, StandardCharsets.ISO_8859_1));
    }

    public static double parseDouble(byte[] str, int offset, int length) throws java.lang.NumberFormatException {
        ...
        fastPath: for (;;) {
            ...
            if (hasLeadingZero) {
                ...
                if (ch == 'x' || ch == 'X') {
                    ...
                    long significand = 0;
                    int exponent = 0;
                    final int significandStartIndex = index1;
                    ...
                    for (; index1 < endIndex; index1++) {
                        ch1 = str[index1];
                        int hexValue = ch1 < 0 ? OTHER_CLASS : CHAR_TO_HEX_MAP[ch1];
                        if (hexValue >= 0) {
                            significand = (significand << 4) | hexValue;
                        }
                        else if (hexValue == DECIMAL_POINT_CLASS) {
                            illegal |= virtualIndexOfPoint >= 0;
                            virtualIndexOfPoint = index1;
                        }
                        else {
                            break;
                        }
                    }
                    final int significandEndIndex = index1;
                    if (virtualIndexOfPoint < 0) {
                        ...
                        exponent = Math.min(virtualIndexOfPoint - index1 + 1, MAX_EXPONENT_NUMBER) * 4;
                    }

                    int expNumber = 0;
                    final boolean hasExponent = (ch1 == 'p') || (ch1 == 'P');
                    if (hasExponent) {
                        ch1 = ++index1 < endIndex ? str[index1] : 0;
                        boolean isExponentNegative = ch1 == '-';
                        if (isExponentNegative || ch1 == '+') {
                            ch1 = ++index1 < endIndex ? str[index1] : 0;
                        }
                        illegal |= !isDigit(ch1);
                        do {
                    ...
                    if (ch1 == 'd' | ch1 == 'D' | ch1 == 'f' | ch1 == 'F') {
                        index1++;
                    }

                    index1 = skipWhitespace(str, index1, endIndex);
                    if (illegal || index1 < endIndex || digitCount == 0 || !hasExponent) {
                        throw new java.lang.NumberFormatException();
                    }
                    ...
                    if (digitCount > 16) {
                        significand = 0;
                        for (index1 = significandStartIndex; index1 < significandEndIndex; index1++) {
                            ch1 = str[index1];
                            int hexValue = ch1 < 0 ? OTHER_CLASS : CHAR_TO_HEX_MAP[ch1];
                            if (hexValue >= 0) {
                        ...
                        isSignificandTruncated = (index1 < significandEndIndex);
                    } else {
                        isSignificandTruncated = false;
                    }

                    long exponentOfTruncatedSignificand = virtualIndexOfPoint - index1 + skipCountInTruncatedDigits + expNumber;
                    ...
                    if (isSignificandTruncated) {
                        if (Double.MIN_EXPONENT <= exponentOfTruncatedSignificand && exponentOfTruncatedSignificand <= Double.MAX_EXPONENT) {
                            ...
                            if (!isNaN(withoutRounding) && roundedUp == withoutRounding) {
                    ...
                    else if (Double.MIN_EXPONENT <= exponent && exponent <= Double.MAX_EXPONENT) {
                        var fastResult = tryHexFloatToDoubleWithFastAlgorithm(isNegative, significand, (int) exponent);
                        if (isNaN(fastResult)) {
                            break fastPath;
                        }
                        else {
            ...
            for (; index1 < endIndex; index1++) {
                ...
                if (isDigit(ch1)) {
                    ...
                    illegal |= virtualIndexOfPoint >= 0;
                    virtualIndexOfPoint = index1;
                    for (; index1 < endIndex - 4; index1 += 4) {
                        int digits = tryToParseFourDigits(str, index1 + 1);
                        if (digits < 0) {
                            break;
                        }
            ...
            int exponent;
            if (virtualIndexOfPoint < 0) {
                digitCount = index1 - significandStartIndex;
                virtualIndexOfPoint = index1;
                exponent = 0;
            } else {
                digitCount = index1 - significandStartIndex - 1;
                exponent = virtualIndexOfPoint - index1 + 1;
            }
            ...
            index1 = skipWhitespace(str, index1, endIndex);
            if (illegal || index1 < endIndex || !hasLeadingZero && digitCount == 0) {
                throw new java.lang.NumberFormatException();
            }
            ...
            if (digitCount > 19) {
                ...
                isSignificandTruncated = (index1 < significandEndIndex);
                exponentOfTruncatedSignificand = virtualIndexOfPoint - index1 + skipCountInTruncatedDigits + expNumber;
            }
            else {
                isSignificandTruncated = false;
            ...
            if (isSignificandTruncated) {
                if (MIN_EXP_POWER_OF_TEN <= exponentOfTruncatedSignificand && exponentOfTruncatedSignificand <= MAX_EXP_POWER_OF_TEN) {
            ...
            else if (MIN_EXP_POWER_OF_TEN <= exponent && exponent <= MAX_EXP_POWER_OF_TEN) {
                var fastResult = tryDecFloatToDoubleWithFastAlgorithm(isNegative, significand, exponent);
                if (isNaN(fastResult)) {
                    break fastPath;
                }
                else {
        ...
        return slowPath(str, offset, endIndex - offset);
    }
}
