    ...
    public static double parseDouble(byte[] str, int offset, int length) throws NumberFormatException {
        ...
        if (hasLeadingZero) {
            ...
            if (ch == 'x' || ch == 'X') {
                ...
                double d = tryHexFloatToDoubleTruncated(isNegative, significand, exponent, isSignificandTruncated, virtualIndexOfPoint - index1 + skipCountInTruncatedDigits + expNumber);
                return Double.isNaN(d) ? Double.parseDouble(new String(str, offset, endIndex - offset, StandardCharsets.ISO_8859_1)) : d;
            }
        }
        ...
        double d = tryDecFloatToDoubleTruncated(isNegative, significand, exponent, isSignificandTruncated, exponentOfTruncatedSignificand);
        return Double.isNaN(d) ? Double.parseDouble(new String(str, offset, endIndex - offset, StandardCharsets.ISO_8859_1)) : d;
    }
}
